...
final class BC7Decoder extends BCDecoder {
    ...
    public void decodeBlock(byte[] src, int srcPos, byte[] dst, int dstPos, int stride) {
        ...
        for (int y = 0, i = 0; y < BLOCK_HEIGHT; y++) {
            for (int x = 0; x < BLOCK_WIDTH; x++, i++) {
                ...
                int r = interpolate(c0[0], c1[0], cWeight);
                int g = interpolate(c0[1], c1[1], cWeight);
                int b = interpolate(c0[2], c1[2], cWeight);
                int a = interpolate(c0[3], c1[3], aWeight);
    ...
    private static final class Mode {
        ...
        private Mode(int ns, int pb, int rb, boolean isb, int cb, int ab, boolean epb, boolean spb, int ib1, int ib2) {
            this.ns = ns;
            this.pb = pb;
            this.rb = rb;
            this.isb = isb;
