...
class BC6HFloatTest {
    ...
    void testBC6H_UF16() throws IOException {
        ...
        BlockDecoder decoder = BlockDecoder.bc6hFloat(false);
        ...
        BCTestUtils.assertBufferEqualsHalfFloats(actual, expected);
    }
    ...
    void testBC6H_SF16() throws IOException {
        ...
        BlockDecoder decoder = BlockDecoder.bc6hFloat(true);
        ByteBuffer actual = decoder.decode(src.position(BCTestUtils.DDS_HEADER_SIZE), 256, 256);
    ...
    void testBC6HInvalidBlock() {
        ...
        for (byte invalidMode : invalidModes) {
            ...
            ByteBuffer expected = ByteBuffer.allocate(16 * BC6HFloat.BPP);
            assertThat(actual).isEqualTo(expected);
        }
    }

    @Test
    void testMatchesHalfFloatDecoder() throws IOException {
        ByteBuffer src = BCTestUtils.readResource("/bc6h_uf16.dds");

        ByteBuffer halfs = BlockDecoder.bc6h(false)
            .decode(src.position(BCTestUtils.DDS_HEADER_SIZE), 256, 256);
        ByteBuffer floats = BlockDecoder.bc6hFloat(false)
            .decode(src.position(BCTestUtils.DDS_HEADER_SIZE), 256, 256);

        BCTestUtils.assertBufferEqualsHalfFloats(floats, halfs);
    }
