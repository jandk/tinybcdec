...
class BC5UDecoderTest {
    ...
    void testBC5U() throws IOException {
        ...
        byte[] actual = BlockDecoder.create(BlockFormat.BC5Unsigned, PixelOrder.BGR)
    ...
    void testBC5UNormalized() throws IOException {
        ...
        byte[] actual = BlockDecoder.create(BlockFormat.BC5UnsignedNormalized, PixelOrder.BGR)
            .decode(256, 256, src, BCTestUtils.DDS_HEADER_SIZE);
        byte[] expected = BCTestUtils.readPng("/bc5u_normalized.png");

        for (int i = 0; i < expected.length; i += 3) {
            assertThat(actual[i + 2] & 0xFF).isEqualTo(expected[i + 2] & 0xFF);
            assertThat(actual[i + 1] & 0xFF).isEqualTo(expected[i + 1] & 0xFF);
            int za = actual[i] & 0xFF;
            int ze = expected[i] & 0xFF;
            if (ze != 0) {
