...
final class GenerateOpcodes {
    ...
    public static void main(String[] args) {
        ...
        for (String line : opcodes.split("\\R")) {
            for (String s : line.substring(line.indexOf(' ') + 1).split(", ")) {
                ...
                int shift = Math.min(range1, range2);
                int count = Math.max(range1, range2) - shift + 1;
                int reversed = range1 < range2 ? 1 : 0;
